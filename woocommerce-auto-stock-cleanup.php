...
class WooCommerce_Auto_Stock_Cleanup {
    ...
    public function rest_get_out_of_stock_products( $request ) {
        ...
        if ( $filter === 'out-of-stock' ) {
            $products = $this->get_all_out_of_stock_products( $limit );
    ...
    public function enqueue_assets( $hook ) {
        if ( $hook !== 'tools_page_delete-images-by-ids' )
            return;
            ...
            <div style="background: #fff; padding: 20px; border: 1px solid #ccc; margin-bottom: 30px;">
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <?php else : ?>
                        ...
                        <div
                            style="background: #f0f0f1; padding: 10px; border-radius: 4px; font-family: monospace; word-break: break-all;">
                ...
                <div
                    style="background: #fff3cd; padding: 15px; border: 1px solid #ffc107; border-radius: 4px; margin-bottom: 20px;">
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <ul>
                        ...
                        <li><code>filter</code> (optional) - Use <code>filter=out-of-stock</code> to get all out-of-stock
                            products from ALL categories</li>
                        <li><code>limit</code> (optional, default 100, max 1000) - Number of products</li>
                    </ul>
                    <p><strong>Examples:</strong></p>
                    <pre
                        style="background: #282c34; color: #abb2bf; padding: 15px; border-radius: 4px; overflow-x: auto;"># Get 10 products with fewer than 5 variations from biustonosze category
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <ul>
                        <li><code>cat</code> (optional) - Category slug filter (e.g., 'biustonosze')</li>
                        <li><code>filter</code> (optional) - Use <code>filter=out-of-stock</code> to delete all out-of-stock
                            products from ALL categories</li>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <pre
                        style="background: #282c34; color: #abb2bf; padding: 15px; border-radius: 4px; overflow-x: auto;">0 2 * * * curl -X POST "<?php echo esc_html( $site_url ); ?>/wp-json/delete-images/v1/cleanup" -H "X-API-Key: YOUR_API_KEY" >> /var/log/product-cleanup.log 2>&1</pre>
                </div>
                ...
                <div>
                    ...
                    <p><strong>Example cURL:</strong></p>
                    <pre
                        style="background: #282c34; color: #abb2bf; padding: 15px; border-radius: 4px; overflow-x: auto;">curl "<?php echo esc_html( $site_url ); ?>/wp-json/delete-images/v1/stats"</pre>
                </div>
            </div>
    ...
    /**
     * Get single quantity products OR category-filtered products (optimized query)
     */
    private function get_single_quantity_products( $category = null, $limit = 100 ) {
        global $wpdb;

        if ( $category ) {
            // 🎯 CATEGORY FILTER MODE (e.g. 'biustonosze')
            $query = $wpdb->prepare( "
            SELECT 
                p.ID AS product_id,
    ...
    /**
     * Get all out-of-stock products (simple + variable)
     */
    private function get_all_out_of_stock_products( $limit = 100 ) {
        global $wpdb;
        ...
        WHERE 
            ...
            AND tt.taxonomy = 'product_cat'
            AND (
                -- Simple products directly out of stock
                EXISTS (
                ...
                OR
                -- Variable products where all variations are out of stock
                p.ID IN (
                    SELECT parent.ID
                    FROM {$wpdb->prefix}posts AS parent
                    JOIN {$wpdb->prefix}posts AS v 
    ...
    /**
     * Get brazyliany (biustonosze) products with FEWER THAN 5 variations (sizes)
     */
    ...
    /**
     * Count brazyliany (biustonosze) products with FEWER THAN 5 variations (sizes)
     */
